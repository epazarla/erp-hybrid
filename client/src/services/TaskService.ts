...
// Sabitler
export const TASKS_STORAGE_KEY = 'erp_tasks_v7'; // Versiyon numarasını artırdık, yeni durum değerleri için
...
export const saveTasksToStorage = (tasks: Task[]): boolean => {
  const result = _saveTasksToStorageInternal(tasks);
  
  if (result) {
    // Görevler güncellendiğinde olayı tetikle
    window.dispatchEvent(new CustomEvent(TASKS_UPDATED_EVENT));
    console.log(`[${new Date().toISOString()}] Görev güncelleme olayı tetiklendi`);
  }
