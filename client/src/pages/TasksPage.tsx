...
const TasksPage: React.FC = () => {
  ...
  const [newTask, setNewTask] = useState<Omit<Task, 'id' | 'created_at'> & { clientId?: string }>({ 
    title: '', 
    description: '', 
    assigned_to: 0, 
    status: 'Bekliyor', 
  ...
  const loadTasks = () => {
    try {
      ...
      // Kategorileri topla
      const uniqueCategories = Array.from(new Set(allTasks.filter(task => task.category).map(task => task.category as string)));
  ...
  const handleAddTask = () => {
    ...
    try {
      // clientId'yi task nesnesine eklemiyoruz, sadece kategori alanını kullanıyoruz
      // Eğer bir müşteri seçilmişse ve kategori boşsa, müşteri adını kategori olarak kullan
      let taskToAdd = { ...newTask };
      
      if (newTask.clientId && !newTask.category) {
        const selectedClient = activeClients.find(client => client.id === newTask.clientId);
        if (selectedClient) {
          taskToAdd.category = selectedClient.name;
        }
      }
      
      // clientId'yi kaldır çünkü Task tipinde yok
      const { clientId, ...taskWithoutClientId } = taskToAdd;
      
      const result = addTask(taskWithoutClientId);
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 'lg', mx: 'auto' }}>
      ...
      <Dialog open={openDialog} onClose={handleCloseFilterDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            Görevleri duruma ve atanan kişiye göre filtreleyebilirsiniz.
          </DialogContentText>
